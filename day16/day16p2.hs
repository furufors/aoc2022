...
bruteforce (flowMap, distMap) =
    let (score, visited) = maximumBy (comparing fst) [ (run ("AA":order),order) | ns <- subsetLists, order <- permutations ns]
    in score + maximum [ run ("AA":elephant) | elephant <- permutations (names\\visited)]
    --trace (show names) $ maximum [run ("AA":order) + run ("AA":elephant) | ns <- subsetLists, order <- permutations ns, elephant <- permutations (names\\ns)]
    where
        subsetLists = subsets 6 names
        names = filter (/="AA") . nub $ M.keys flowMap
        run :: [String] -> Int
        run order = run' 26 order
